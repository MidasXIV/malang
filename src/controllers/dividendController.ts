...
export class DividendController {
    ...
    private parseStockSummary($: CheerioStatic): StockSummaryInterface {
        ...
        const defaultStringValue = 'value Not Found', defaultNumericValue = -1;

        let marketCap: string = defaultStringValue, beta: number = defaultNumericValue,
            peRatio: number = defaultNumericValue, EPS: number = defaultNumericValue,
            paymentDate: string = defaultStringValue, exDividendDate: string = defaultStringValue,
            dividendAmount: number = defaultNumericValue, dividendYeild: string = defaultStringValue,
            dividendPayoutRatio: number = defaultNumericValue;

        $(this.stockSummaryQuerySelector).children("tr").each(function (index, row) {

            const rowInfo = $(row).children("td").map(function (_index, col) {
                return $(col).text();
            }).get();

            switch (rowInfo[0]) {
                case "Market Cap": marketCap = rowInfo[1]; break;
                case "Beta (5Y Monthly)": beta = parseFloat(rowInfo[1]); break;
                case "PE Ratio (TTM)": peRatio = parseFloat(rowInfo[1]); break;
                case "EPS (TTM)": EPS = parseFloat(rowInfo[1]); break;
                case "Earnings Date": paymentDate = rowInfo[1]; break;
                case "Ex-Dividend Date": exDividendDate = rowInfo[1]; break;
        ...
        return { marketCap, beta, peRatio, EPS, paymentDate, exDividendDate, dividendAmount, dividendYeild, dividendPayoutRatio }
    }
    ...
    private parseDividendHistory($: CheerioStatic): Array<DividendInformationItemInterface> {
        ...
        $(this.dividendHistoryTableQuerySelector).children("tr").each(function (index, row) {
    ...
    private getAnnualDividends(DividendHistory: Array<DividendInformationItemInterface>, dividendAmount: number): AnnualDividendInterface {
        ...
        DividendHistory.forEach((DividendHistoryItem: DividendInformationItemInterface) => {
            ...
            AnnualDividends[dividendYear] ? AnnualDividends[dividendYear] += DividendHistoryItem.dividend : AnnualDividends[dividendYear] = DividendHistoryItem.dividend;
        });

        /** if dividendAmount is not provided remove it from Annual Dividend Object */
        if (dividendAmount) { AnnualDividends[currentYear] = dividendAmount; }
        else { delete AnnualDividends[currentYear]; }

        return AnnualDividends;
    }

    private getAnnualDividendGrowth(AnnualDividends: AnnualDividendInterface): AnnualDividendGrowthInterface {
        ...
        const AnnualDividendYearsArray = Object.keys(AnnualDividends).map(Number).sort((a: number, b: number) => a - b);
    ...
    public getDividendInfo(request: Request, response: Response): void {
        ...
        this.makeRequest(yahooFinancedividendProfileURL).then((html) => {
            ...
            response.json({ symbol, name, price, exchange, ...stockSummary, dividendCurrency, AnnualDividends, AnnualDividendGrowth });
        });
    }
